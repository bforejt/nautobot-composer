...
| `FIRMWARE_BASE_URL` | *(derived)* | Device-facing base URL for firmware downloads, e.g. `https://192.0.2.10:9443/images/`. Not used by the firmware containers — `.env` is the `env_file` for every Nautobot container, so it reaches the **Celery worker**, where the *nautobot-upgrades* "Register IOS-XE Image" job builds `download_url` as `<base>/<filename>`. `setup.sh` derives it from the host's **primary IP** (routing table — never DNS or `hostname`, since lab devices dial back by IP); pin it explicitly with `./setup.sh --firmware-url <url>`. Empty ⇒ the job requires a per-run URL instead of guessing. |
...
`FIRMWARE_BASE_URL` is the exception — it is consumed by the **Celery worker**, not the firmware services, so after changing it refresh the core stack instead (`docker compose up -d` re-creates containers whose environment changed).
...
**How the job finds the base URL:** `.env` is the `env_file` for every Nautobot container, so the `FIRMWARE_BASE_URL` value that `setup.sh` writes there (host primary IP + `FIRMWARE_HTTPS_PORT`, e.g. `https://192.0.2.10:9443/images/`) lands in the Celery worker's environment, where the job reads it. The job's per-run *Firmware base URL* field can normally stay blank; fill it (or the full *Download URL override*) only to deviate for a single run. If `FIRMWARE_BASE_URL` changes, run `docker compose up -d` so the worker container picks up the new value.
